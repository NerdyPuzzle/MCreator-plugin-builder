...
void TextEditor::SetPalette(PaletteId aValue)
{
	...
	case PaletteId::RetroBlue:
		...
		break;
	case PaletteId::VisualStudio:
		palletteBase = &(GetVisualStudioPalette());
		break;
	case PaletteId::Darcula:
		palletteBase = &(GetDarculaPalette());
		break;
	}
...
void TextEditor::SetLanguageDefinition(LanguageDefinitionId aValue)
{
	...
	case LanguageDefinitionId::None:
		...
		return;
	case LanguageDefinitionId::JavaFreemarker:
		mLanguageDefinition = &(LanguageDefinition::JavaFreemarker());
		break;
	}
...
const TextEditor::Palette& TextEditor::GetVisualStudioPalette()
{
	const static Palette p = { {
		0xF0F0F0ff, // Default
		0x569CD6ff, // Keyword
		0xB5CEA8ff, // Number
		0xCE9178ff, // String
		0xD4D4D4ff, // Char literal
		0x4EC9B0ff, // Punctuation
		0xD4D4D4ff, // Preprocessor
		0xD4D4D4ff, // Identifier
		0x569CD6ff, // Known identifier
		0x4EC9B0ff, // Preproc identifier
		0x6A9955ff, // Comment (single line)
		0x6A9955ff, // Comment (multi line)
		0x1E1E1Eff, // Background
		0xFFFFFFFF, // Cursor
		0x264F78ff, // Selection
		0xFF0000ff, // ErrorMarker
		0x000000ff, // ControlCharacter
		0xFFFFFF00, // Breakpoint
		0xB5CEA8ff, // Line number
		0x3C3C3C80, // Current line fill
		0x3C3C3C80, // Current line fill (inactive)
		0x515151ff, // Current line edge
	} };
	return p;
}

const TextEditor::Palette& TextEditor::GetDarculaPalette()
{
	const static Palette p = { {
		0x2b2b2bff, // Default
		0x9876aaff, // Keyword
		0x6897bbff, // Number
		0x6a8759ff, // String
		0x6a8759ff, // Char literal (same as String in Darcula)
		0xbbb529ff, // Punctuation
		0xcc7832ff, // Preprocessor
		0xbbbbbbff, // Identifier
		0x6897bbff, // Known identifier (same as Number in Darcula)
		0x9876aaff, // Preproc identifier (same as Keyword in Darcula)
		0x808080ff, // Comment (single line)
		0x808080ff, // Comment (multi line)
		0x3c3f41ff, // Background
		0xffffffff, // Cursor
		0x214283ff, // Selection
		0xff0000ff, // ErrorMarker
		0x808080ff, // ControlCharacter
		0xff0000ff, // Breakpoint
		0x808080ff, // Line number
		0x3a3a3a80, // Current line fill
		0x3a3a3a80, // Current line fill (inactive)
		0x515151ff, // Current line edge
	} };
	return p;
}

const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::JavaFreemarker()
{
	...
	if (!inited)
	{
		static const char* const javaKeywords[] = {
			...
			"try", "void", "volatile", "while", "null", "Override", "SubscribeEvent", "Mod", "EventBusSubscriber"
		};
		for (auto& k : javaKeywords)
			langDef.mKeywords.insert(k);

		static const char* const freemarkerKeywords[] = {
			"assign", "attempt", "break", "case", "compress", "default", "else", "elseif", "escape", "fallback", "flush",
			"foreach", "ftl", "function", "global", "if", "import", "include", "list", "local", "macro", "nested",
			"noescape", "recover", "recurse", "return", "setting", "stop", "switch", "visit", "as"
		};
		for (auto& k : freemarkerKeywords)
		{
			Identifier id;
			id.mDeclaration = "Freemarker keyword";
		...
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>(R"##(0[xX][0-9a-fA-F]+[Ll]?)##", PaletteIndex::Number));
...
TextEditor::PaletteId TextEditor::defaultPalette = TextEditor::PaletteId::Darcula;